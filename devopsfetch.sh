...
# Uses netstat to list all active ports along with the user and service associated with each port
display_ports() {
    printf "%-25s | %-25s | %-25s\n" "User" "Port" "Service"
    printf "%-25s | %-25s | %-25s\n" "--------------" "--------------" "--------------"
...
# Takes a port number as an argument and displays the user and service associated with the specified port
display_port_info() {
    local port=$1
    printf "%-25s | %-25s | %-25s\n" "User" "Port" "Service"
    printf "%-25s | %-25s | %-25s\n" "--------------" "--------------" "--------------"
...
# Function to display all NGINX server domains, proxies, and their configuration files
# Searches through NGINX configuration files to list domains and their corresponding proxy settings
display_nginx() {
...
# Function to display detailed configuration information for a specific NGINX domain
# Takes a domain name as an argument and displays the port and configuration file associated with the domain
display_nginx_info() {
    local domain=$1
...
# Reads from the /etc/passwd file and uses lastlog to get the last login time for each user
list_users() {
    printf "%-20s | %-20s\n" "Username" "Last Login"
    printf "%-20s | %-20s\n" "--------------------" "--------------------"
...
# Takes a username as an argument and displays the last login time for that user
user_info() {
    local username=$1
    printf "%-20s | %-20s\n" "Username" "Last Login"
    printf "%-20s | %-20s\n" "--------------------" "--------------------"
...
# Uses docker commands to list all Docker images and their details, and all containers with their status
list_docker() {
    echo "Docker Images:"
...
# Takes a container ID as an argument and displays the status of the container
container_info() {
...
# Takes a date (YYYY-MM-DD) as an argument and displays log activities within that date
display_time_range() {
    local date=$1
...
# Shows the usage of the script and describes the available options
print_help() {
    echo -e "Usage: $0 [options]"
    echo -e "Options:"
    echo -e "  -p, --port [number]     List active ports, services, and users"
    echo -e "  -d, --docker [id]       List Docker images and containers, or details of a specific container"
    ...
    echo -e "  -t, --time YYYY-MM-DD   Display activities within a specified time range"
